...
import { ArrowLeft, Wallet, Shield, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export default function BindWalletPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (!walletAddress.trim()) {
            setMessage({ type: 'error', text: 'Please enter a valid TRC20 wallet address' });
            return;
        }

        // Basic TRC20 validation (starts with T and is 34 chars long)
            ...
            <div className="relative z-10 space-y-6 max-w-md mx-auto">
                ...
                <div className="glass-card p-6 animate-slide-up" style={{ animationDelay: '0.05s' }}>
                    <div className="flex justify-center mb-6">
                        <div className="w-16 h-16 rounded-full bg-primary/20 flex flex-col items-center justify-center border-2 border-primary/30 shadow-[0_0_30px_rgba(var(--primary),0.3)]">
                            <Wallet size={32} className="text-primary-light" />
                        </div>
                    </div>

                    <h2 className="text-xl font-bold text-text-primary text-center mb-2">USDT Wallet Address</h2>
                    <p className="text-sm text-text-secondary text-center mb-8">
                        Please bind your TRC20 network wallet address for receiving withdrawals.
                    </p>

                    <form onSubmit={handleSave} className="space-y-6">
                        <div className="relative group">
                            <label className="text-xs font-medium text-text-secondary uppercase tracking-wider mb-2 block">
                                TRC20 Network Address
                            </label>
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="btn-primary w-full py-4 text-lg font-bold flex justify-center items-center shadow-[0_0_20px_rgba(var(--primary),0.4)] hover:shadow-[0_0_30px_rgba(var(--primary),0.6)]"
                        >
                            {isLoading ? <Loader2 size={24} className="animate-spin text-text-primary" /> : 'Save Wallet'}
                        </button>
                    </form>
                </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <div className="text-xs text-text-secondary leading-relaxed">
                            ...
                            <ul className="list-disc pl-4 space-y-1">
                                <li>Ensure the network is exactly USDT-TRC20.</li>
